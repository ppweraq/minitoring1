...
# Calculate the number of configuration files
# config_files=$(get_file_count_by_extension "$1" "conf")

# Calculate the number of text files
...
# Calculate the number of archive files
# archive_files=$(get_file_count_by_extension "$1" "zip")

# Calculate the number of symbolic links
# symbolic_links=$(find "$1" -type l | wc -l)

# Get the top 10 files with the highest weight
# top_files=$(get_top_files "$1" 10)

# Get the top 10 executable files with the largest weight
...
echo "Number of:"
echo "Configuration files (with the .conf extension) = $(get_file_count_by_extension "$1" "conf")s"
...
echo "Archive files = $(get_file_count_by_extension "$1" "zip")"
echo "Symbolic links = $(find "$1" -type l | wc -l)"
echo "Top 10 files of maximum size arranged in descending order (path, size and type):"
echo "$(get_top_files "$1" 10)"
echo "Top 10 executable files of the maximum size arranged in descending order (path, size and MD5 hash of file):"
echo "$(get_top_executables "$1" 10)"
end_time=$(date +%s.%N)
execution_time=$(echo "$end_time - $start_time" | bc)
